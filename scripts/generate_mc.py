...
def print_banner():
  ...
  {C.DARK_GRAY}https://github.com/Jarvis322/macoscode • Build {VERSION_CODE} • Tahoe Ready{C.RESET}
"""
    print(banner)
...
def update_mc(interactive=True, force=False):
    if interactive:
        clear_screen()
        print_banner()
    print(f"{C.BOLD}{C.CYAN}🔄  'mc' Sürüm ve Güncelleme Denetimi (Self-Update){C.RESET}\n")
    print(f"Mevcut Sürüm: {C.BOLD}{C.EMERALD}v{VERSION}{C.RESET} (Build {VERSION_CODE})")
    print(f"{C.GRAY}GitHub üzerindeki en son sürüm sorgulanıyor...{C.RESET}\n")
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=12) as response:
            if response.status != 200:
                print(f"\n{C.ROSE}❌ Güncelleme indirilemedi: HTTP {response.status}{C.RESET}")
                if interactive:
                    print(f"\n{C.GRAY}Devam etmek için bir tuşa basın...{C.RESET}")
                    get_key()
                return
        ...
        print(f"\n{C.ROSE}❌ Bağlantı hatası: {e}{C.RESET}")
        if interactive:
            print(f"\n{C.GRAY}Devam etmek için bir tuşa basın...{C.RESET}")
            get_key()
        return
    ...
    if "#!/usr/bin/env python3" not in new_code or len(new_code) < 1000:
        print(f"\n{C.ROSE}❌ İndirilen dosya geçerli bir macOSCode betiği değil.{C.RESET}")
        if interactive:
            print(f"\n{C.GRAY}Devam etmek için bir tuşa basın...{C.RESET}")
            get_key()
        return
    ...
    rem_ver_match = re.search(r'VERSION\s*=\s*["\']([^"\']+)["\']', new_code)
    ...
    print(f"En Son GitHub Sürümü: {C.BOLD}{C.CYAN}v{remote_version}{C.RESET} (Build {remote_vcode})\n")

    if remote_vcode > 0 and remote_vcode <= VERSION_CODE and not force:
        print(f"{C.EMERALD}✔ Zaten en güncel sürümü (v{VERSION}) kullanıyorsunuz!{C.RESET}")
        if interactive:
            print(f"\n{C.GOLD}Yine de zorla yeniden yüklemek ister misiniz? (e/h):{C.RESET} ", end="")
            choice = get_key()
    ...
    if remote_vcode > VERSION_CODE:
        print(f"{C.GOLD}🚀 Yeni bir sürüm bulundu: {C.ROSE}v{VERSION}{C.GOLD} ➜ {C.EMERALD}v{remote_version}{C.RESET}")
        print(f"{C.CYAN}Güncelleme uygulanıyor...{C.RESET}\n")
    ...
    for target in targets:
        try:
            ...
            print(f"{C.EMERALD}✔ '{target}' güncellendi (v{remote_version}).{C.RESET}")
            updated_any = True
        except PermissionError:
            print(f"{C.GOLD}🔐 '{target}' güncellemesi için yönetici (sudo) izni isteniyor...{C.RESET}")
            res = subprocess.run(f"sudo cp '{temp_path}' '{target}' && sudo chmod 755 '{target}'", shell=True)
            if res.returncode == 0:
                print(f"{C.EMERALD}✔ '{target}' güncellendi (v{remote_version}, sudo ile).{C.RESET}")
                updated_any = True
            else:
                print(f"{C.ROSE}❌ '{target}' güncellenemedi.{C.RESET}")
        except Exception as e:
            print(f"{C.ROSE}❌ '{target}' güncellenirken hata: {e}{C.RESET}")
    ...
    if updated_any:
        print(f"\n{C.BOLD}{C.PURPLE}🎉 Başarılı! 'mc' v{remote_version} (Build {remote_vcode}) sürümüne yükseltildi! 🚀{C.RESET}")
    else:
        print(f"\n{C.GOLD}⚠️ Hedef dosyalar güncellenemedi.{C.RESET}")
...
def run_main_tui():
    ...
    menu_items = [
        ...
        ("[9]", "🔄", "'mc' Aracını Güncelle (Self-Update from GitHub)", "update"),
        ("[0]", "🚪", "Çıkış (Quit)", "quit")
    ]
    ...
    while True:
        ...
        print(f"\n{C.DARK_GRAY}Navigasyon: [↑/↓] Gezin • [Enter/Space] Seç • [1-9/0] Hızlı Tuş • [q] Çıkış{C.RESET}")
...
def main():
  ...
  mc --update            'mc' komutunu GitHub'dan en son sürüme günceller
  mc --version           Sürüm bilgisini gösterir (v{VERSION})
"""
    )
    ...
    parser.add_argument("-u", "--update", action="store_true", help="'mc' aracını GitHub'dan en son sürüme güncelle")
    parser.add_argument("-f", "--force", action="store_true", help="Sürüm aynı olsa bile güncellemeyi zorla")
