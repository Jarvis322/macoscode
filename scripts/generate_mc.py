...
def update_mc(interactive=True):
    if interactive:
        clear_screen()
        print_banner()
    print(f"{C.BOLD}{C.CYAN}🔄  'mc' Otomatik Güncelleme (Self-Update){C.RESET}\n")
    print(f"{C.GRAY}GitHub üzerindeki en son sürüm kontrol ediliyor...{C.RESET}")
    ...
    for target in targets:
        try:
            ...
            print(f"{C.EMERALD}✔ '{target}' başarıyla güncellendi.{C.RESET}")
            updated_any = True
        except PermissionError:
            ...
            if res.returncode == 0:
                print(f"{C.EMERALD}✔ '{target}' başarıyla güncellendi (sudo ile).{C.RESET}")
                updated_any = True
            else:
    ...
    if updated_any:
        print(f"\n{C.BOLD}{C.PURPLE}🎉 Tebrikler! 'mc' başarıyla en güncel sürüme yükseltildi! 🚀{C.RESET}")
    else:
